...
function incidentCommand(args) {
  if (args[0] !== 'create') {
    throw new Error('Usage: execfence incident create --from-report <report.json>');
  }
  const reportPath = readOption(args, '--from-report');
  if (!reportPath) {
    throw new Error('Usage: execfence incident create --from-report <report.json>');
  }
  const output = incidentFromReport(process.cwd(), reportPath);
...
function enrichCommand(args) {
  ...
  if (!reportPath) {
    throw new Error('Usage: execfence enrich <report.json>');
  }
...
function publishPackage(options = {}) {
  ...
  run('npm', ['run', 'scan']);
  run('node', ['bin/execfence.js', 'manifest']);
