...
function redactionPreview(findings = [], config = {}) {
  return {
    status: 'preview',
    redactLocalPaths: config.redaction?.redactLocalPaths !== false,
    findings: findings.map((finding) => {
      const indicators = indicatorsFor(finding);
      return {
        findingId: finding.id,
        originalFile: finding.file || null,
        redactedFile: redactPath(finding.file || '', config),
        queries: redactedQueries(finding, indicators, config),
        indicators,
      };
    }),
  };
...
function redactedQueries(finding, indicators, config = {}) {
  const parts = [
    finding.id,
    ...indicators.ruleIds,
    ...indicators.packages,
  ...
  if (parts.length === 0) {
    parts.push('ExecFence security finding');
  }
  return Array.from(new Set(parts.map((item) => redactText(item, config)))).slice(0, config.analysis?.webEnrichment?.maxQueriesPerFinding || 3);
}

function redactPath(value, config = {}) {
  if (config.redaction?.redactLocalPaths === false) {
    return value;
  }
  return String(value || '').replace(/[A-Za-z]:[\\/][^\s"']+/g, '[local-path]').replace(/\/(?:Users|home)\/[^\s"']+/g, '[local-path]');
}

function redactText(value, config = {}) {
  let text = redactPath(value, config);
  for (const pattern of config.redaction?.extraPatterns || []) {
    try {
      text = text.replace(new RegExp(pattern, 'g'), '[redacted]');
    } catch {
      // Ignore invalid optional redaction patterns.
    }
  }
  return text;
}
